...
class ErgoContractCompilerImpl(val c: MacrosContext) {
  ...
  def liftParam(n: String, tpe: Type, paramMap: Map[String, String]): Expr[SValue] =
    tpe.widen match {
      case ByteTpe =>
        reify(ByteConstant(c.Expr[Byte](Ident(TermName(paramMap(n)))).splice))
      case IntTpe => reify(IntConstant(c.Expr[Int](Ident(TermName(paramMap(n)))).splice))
      case LongTpe =>
        reify(LongConstant(c.Expr[Long](Ident(TermName(paramMap(n)))).splice))
  ...
  private def buildFromScalaAst(
    s: Tree,
    defId: Int,
    paramMap: Map[String, String],
    valDefNameIds: Map[String, (Int, SType)]
  ): Expr[SValue] = {
    ...
    s match {
      ...
      case Select(obj, m) =>
        val o = recurse(obj)
        obj.tpe.widen match {
          case TypeRef(_, sym, _) if sym.fullName == "special.collection.Coll" =>
            m match {
              case TermName("length") => reify(SizeOf(o.splice.asCollection[SType]))
              case TermName("size")   => reify(SizeOf(o.splice.asCollection[SType]))
              case TermName("nonEmpty") =>
                reify(GT(SizeOf(o.splice.asCollection[SType]), IntConstant(0)))
            }
          case TypeRef(_, sym, _) if sym.fullName == "special.sigma.Box" =>
            m match {
              case TermName("tokens") =>
                // TODO check how it's done in sigma
                reify(
                  OptionGet(
                    ExtractRegisterAs(o.splice.asBox, R2, SOption(ErgoBox.STokensRegType))
                  )
                )
          ...
          case TypeRef(_, sym, _) if sym.fullName == "scala.Option" =>
            m match {
        ...
        obj.tpe.widen match {
          case TypeRef(_, sym, _) if sym.fullName == "special.sigma.Box" =>
            m match {
              ...
              case TermName("R4") =>
                reify(ExtractRegisterAs(o.splice.asBox, R4, SOption(SCollection(SByte))))
            }
        }
      ...
      case Apply(Select(lhs, m), Seq(arg)) =>
        ...
        lhs.tpe.widen match {
          ...
          case TypeRef(_, sym1, _)
              if sym1.fullName == "special.sigma.SigmaProp"
              || sym1.fullName.endsWith("BooleanOps") =>
            m match {
              case TermName("$amp$amp") =>
                arg.tpe.widen match {
                  case BooleanTpe =>
                    reify(
                      SigmaAnd(
                        l.splice.asSigmaProp,
                        BoolToSigmaProp(r.splice.asBoolValue)
                      )
                    )
              ...
              case TermName("$bar$bar") =>
                arg.tpe.widen match {
                  case BooleanTpe =>
                    reify(
                      SigmaOr(l.splice.asSigmaProp, BoolToSigmaProp(r.splice.asBoolValue))
                    )
          ...
          case _ =>
            error(s"object $lhs(tpe: ${lhs.tpe.widen}) has unexpected $m with arg: $arg")
        }
  ...
  def compile[A, B](verifiedContract: c.Expr[A => B]): c.Expr[ErgoContract] = {
    ...
    reify(
      ErgoContract(c.Expr[Context => SigmaProp](contractTree).splice, sigmaProp.splice)
    )
  }
